...
| history | `{ role: "user" \| "assistant", content: string }[]` | N | 이전 대화 턴(최근 순, 최대 20개). 멀티턴 문맥 유지를 위해 클라이언트가 화면에 쌓아둔 이력을 함께 보낸다. 서버는 이 값을 저장하지 않고 health-ai 호출에만 그대로 전달한다(무상태). |
...
- `history`는 이 API가 상태를 갖지 않도록(stateless) 매 요청마다 클라이언트가 전송한다. 서버·DB에 대화 이력을 별도로 저장하지 않는다.
