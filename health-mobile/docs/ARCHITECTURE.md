...
| 차트 | `recharts` (`health-web`과 동일). 순수 SVG/DOM 렌더링이라 RN 컴포넌트 없이도 Web 타겟(react-native-web) 안에서 그대로 동작한다. 네이티브(iOS/Android) 빌드가 실제로 추가되면(8장 TODO) recharts는 DOM 전용이라 쓸 수 없으므로 그때 RN 호환 차트 라이브러리로 교체해야 한다 |
| 스타일링 | **CSS Modules** + [`../../docs/DESIGN.md`](../../docs/DESIGN.md) 토큰 (3장 참고) |
| 인증 상태 | React Context(`AuthContext`), accessToken은 메모리에만 보관, refreshToken은 `health-web`과 동일하게 HttpOnly 쿠키(6장 참고) |
...
- 색상·타이포·간격 등 값은 [`../../docs/DESIGN.md`](../../docs/DESIGN.md)에 정의된 값만 사용하고 임의로 새 값을 만들지 않는다(`health-web`과 동일 문서를 공유하며, `health-web/CLAUDE.md`가 임시로 쓰던 `DESIGN-apple.md`의 값을 그대로 옮겨 확정했다). 필요한 값이 문서에 없으면 먼저 `docs/DESIGN.md`에 추가한 뒤 코드에 반영한다.
- CSS Modules(Metro의 웹 번들링 지원)는 **Web 타겟에서만** 동작한다. 추후 네이티브(iOS/Android) 빌드가 필요해지면 해당 플랫폼의 스타일링 방식을 별도로 결정해 이 장에 반영해야 한다(TODO).
- **중요(RN 컴포넌트에는 `unstable_styles`를 쓴다)**: `*.module.css`의 **default export**(클래스명 문자열 맵)는 `className`을 받는 순수 웹 엘리먼트(`<div>` 등)에만 쓴다. `View`/`Text`/`TextInput`/`Pressable` 같은 React Native 컴포넌트는 `className`을 지원하지 않고(react-native-web이 무시·덮어쓴다) `style` prop만 지원하므로, 반드시 **named export `unstable_styles`**를 `style`에 전달해야 한다.
  ```tsx
  import { unstable_styles as styles } from './Foo.module.css';
  <View style={styles.page}>
  ```
  default export(문자열)를 실수로 `style`에 넘기면 런타임에 `Failed to set an indexed property [0] on 'CSSStyleDeclaration'` 에러로 화면 전체가 크래시한다(문자열을 스타일 객체처럼 순회하다 발생). `app/login.tsx` 구현 중 실제로 겪은 문제이니 새 화면에서 반복하지 않는다.
...
- **중요(Metro는 프로젝트 폴더 밖을 기본적으로 resolve하지 못한다)**: `shared/types.ts`는 타입만 import(`import type`)하면 컴파일 시 제거되어 문제없지만, `shared/utils.ts`처럼 런타임에 실제로 호출되는 값을 import하면 Metro가 `health-mobile/` 밖에 있는 `shared/` 폴더를 resolve하지 못해 `Unable to resolve module`로 번들링이 실패한다(Vite 기반인 health-web은 이런 제약이 없어 겪지 않는 문제). 그래서 `metro.config.js`에 `config.watchFolders = [path.resolve(__dirname, '..', 'shared')]`를 추가해뒀다 — 모노레포 루트 전체(`healthcare/`)를 추가하면 다른 프로젝트의 `node_modules`까지 크롤링해 번들링이 매우 느려지므로 `shared/`만 범위를 좁혀 추가한 것이다. `shared/`를 참조하는 새 코드를 추가할 때 이 설정이 지워지지 않도록 주의한다.
...
> **변경 이력**: 최초 설계는 refreshToken을 `expo-secure-store`에 저장하는 방식이었으나, `API_SPEC.md` 1.1/1.2에 따르면 refreshToken은 응답 바디에 포함되지 않고 **HttpOnly 쿠키로만** 내려간다(클라이언트 JS가 값을 읽을 수 없다). `expo-secure-store`는 저장할 토큰 값 자체를 받을 수 없어 이 설계로는 구현이 불가능했다. health-mobile은 현재 Web 타겟(8장)이 사실상 유일한 실행 환경이므로, `health-web`과 동일하게 브라우저 쿠키 방식으로 통일한다. 네이티브(iOS/Android) 빌드를 실제로 추가하게 되면(8장 TODO) 그때 쿠키 저장이 불가능한 문제를 다시 검토하고 이 장을 갱신한다.

- 로그인(`API_SPEC.md` 1.1) 성공 시 서버가 `accessToken`·`user`를 응답 바디로 반환하고, `refreshToken`은 `Set-Cookie`(HttpOnly)로 내려간다.
- **accessToken**: 응답 바디로 받아 `AuthContext`(메모리)에만 보관한다. API 호출 시 `Authorization: Bearer {accessToken}` 헤더로 전송한다.
- **refreshToken**: 클라이언트가 값을 직접 다루지 않는다. axios 인스턴스에 `withCredentials: true`를 설정해 브라우저가 쿠키를 자동으로 첨부·수신하도록 한다.
- **재발급 흐름**: API 응답이 401이면 axios 인터셉터가 body 없이 `POST /auth/refresh`(1.2)를 호출한다(쿠키가 자동 첨부된다). 새 `accessToken`으로 원 요청을 1회 재시도한다. 재발급마저 실패(쿠키 만료/무효)하면 `AuthContext`를 초기화하고 `/login`으로 이동한다.
- 앱 재실행 시에는 accessToken이 메모리에서 사라지므로, 부트스트랩(루트 `_layout.tsx`) 시 `POST /auth/refresh`를 먼저 호출해 남아있는 refreshToken 쿠키로 accessToken을 재발급받고 로그인 상태를 복원한다. 쿠키가 없거나 재발급에 실패하면 `/login`으로 이동한다.
...
- refreshToken을 `health-web`과 동일하게 HttpOnly 쿠키로 받으므로(6장), backend의 `CORS_ORIGINS` 화이트리스트에 `http://localhost:8081`이 포함돼 있어야 하고 axios는 `withCredentials: true`로 설정한다(와일드카드 `origin: true`와 `credentials: true`는 브라우저가 함께 허용하지 않으므로 반드시 명시적 origin이어야 한다).
